...
{
	fprintf(file, "typedef double (*sigmoidFunction)(double);\n");
	fprintf(file, "double evaluate(long sigmoidPointer) {\n");
	reflect(dag, file);
	fprintf(file,"sigmoidFunction sigmoidCall = (sigmoidFunction)sigmoidPointer;\n");
	fprintf(file, "return sigmoidCall(t%d);\n}\n", dag->id);
}

typedef double (*evaluate_t)(long);
...
main(int argc, char *argv[])
{
	...
	evaluate_t fnc;
	FILE *file;
	long sigmoidPointer = (long)sigmoid;
